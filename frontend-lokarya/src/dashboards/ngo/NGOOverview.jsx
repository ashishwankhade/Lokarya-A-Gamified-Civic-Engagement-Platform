 ...
 *
 * FIXES applied:
 *  [1] Uses useMyMissions hook → correct endpoint, no wasted fetch
 *  [2] Removed unused /pending-approvals fetch that discarded its result
 *  [3] XP calculation uses attendance[].totalPoints (m.volunteerCount doesn't exist)
 *  [4] Uses shared StatusChip component
 */

import React from 'react';
import { motion } from 'framer-motion';
import {
  ListChecks, Users, Zap, TrendingUp,
...
import StatusChip    from '../../components/StatusChip'; // FIX [4]
...
const NGOOverview = ({ onGoToMissions, onGoToCreate, onOpenAttendance }) => {
  // FIX [1] + [2]: single hook, no wasted /pending-approvals call
  ...
  const volunteers = missions.reduce((s, m) =>
    s + (m.attendance?.length || m.participants?.length || 0), 0);

  /* FIX [3]: use attendance[].totalPoints — m.volunteerCount doesn't exist */
  const xpGiven = missions.reduce((s, m) =>
