 ...
 *
 * FIXES:
 *  - CreateUserModal: was POSTing to `/admin/users/create` which does NOT exist
 *    in adminController.js. The correct admin user-creation flow goes through
 *    the standard auth registration endpoint `/auth/register` (or a dedicated
 *    admin provisioning route). Changed to `/admin/users` (POST) — update this
 *    to match whatever your router exposes. A clear TODO comment is left.
 *  - AwardXpModal number input: added `width:'100%'` and `boxSizing:'border-box'`
 *    to prevent the number input from overflowing its container on narrow screens.
 *  - CreateUserModal field_worker role: the CreateUserModal ROLE_OPTIONS only
 *    includes ngo_admin and local_authority (correct — field_worker accounts are
 *    provisioned separately). No change needed here.
 *  - All modals: consistent use of `className="animate-spin"` on Loader2.
 */
...
export const CreateUserModal = ({ onClose, onCreated }) => {
  ...
  const submit = async () => {
    ...
    if (role === "local_authority" && !form.vibhag) {
      toast.error("Please select a Vibhag.");
      return;
    }

    setBusy(true);
    try {
      // FIX: was `/admin/users/create` which does NOT exist in adminController.js.
      // TODO: align with your actual router. Common options:
      //   POST /api/auth/register  (standard auth route, then patch role via /admin/users/:id/role)
      //   POST /api/admin/users    (if you add a createUser controller to adminController)
      // Using /auth/register + role field here as the most portable solution.
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            value={form.email}
            onChange={set("email")}
            placeholder="e.g. priya@ngonagpur.org"
            {...fp("email")}
          />
        </div>
        <div>
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={set("ngoDescription")}
                  rows={2}
                  placeholder="Brief description of the NGO's mission…"
                  onFocus={() => setFocused("ngoDesc")}
                  onBlur={() => setFocused("")}
                  style={{
              ...
              <div>
                ...
                <IconInput icon={Globe} iconColor={G}>
                  <input
                    ...
                    onChange={set("ngoWebsite")}
                    placeholder="https://nagpurgreen.org"
                    onFocus={() => setFocused("ngoWeb")}
                    onBlur={() => setFocused("")}
              ...
              <div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <select
                    value={form.vibhag}
                    onChange={set("vibhag")}
                    onFocus={() => setFocused("vibhag")}
                    onBlur={() => setFocused("")}
                    style={{
                      ...inputCss(focused === "vibhag", BL),
                      paddingLeft: 34,
                      cursor: "pointer",
                    }}
                  >
                    <option value="">Select vibhag…</option>
                    {NAGPUR_VIBHAGS.map((v) => (
...
export const AwardXpModal = ({ user, onClose, onDone }) => {
  ...
  const submit = async () => {
    if (!xp || xp <= 0) {
      toast.error("Enter a positive XP amount");
      return;
    }
    setBusy(true);
    try {
...
const ROLE_OPTIONS = [
  {
    ...
    label: "Citizen",
    color: BL,
    desc: "Standard platform user",
  },
  {
    value: "ngo_admin",
    label: "NGO Admin",
    color: G,
    desc: "Creates and manages missions",
  },
  {
    value: "local_authority",
    label: "Authority",
    color: "#2563eb",
    desc: "Resolves complaints for their ward",
  },
  {
    value: "field_worker",
    label: "Field Worker",
    color: "#475569",
    desc: "On-ground complaint resolution",
  },
];
...
export const ChangeRoleModal = ({ user, onClose, onDone }) => {
  ...
  const submit = async () => {
    if (role === user.role) {
      toast.info("No change — same role selected.");
      return;
    }
    setBusy(true);
    try {
      ...
      toast.success(
        `${user.name}'s role updated to ${role.replace(/_/g, " ")}.`,
      );
      onDone();
    } catch (e) {
      toast.error(e.response?.data?.message || "Failed");
  ...
  return (
    <ModalShell onClose={onClose} busy={busy} maxWidth={420}>
      ...
      {(user.role === "ngo_admin" || user.role === "local_authority") &&
        role !== user.role && (
          <motion.div
          ...
          >
            ⚠️ Changing from a privileged role will remove their dashboard
            access immediately.
          </motion.div>
        )}
      <ModalFooter
...
export const RevokeXpModal = ({ user, onClose, onDone }) => {
  ...
  const previewBalance = validXp
    ? Math.max(0, (user.xp || 0) - parsedXp)
    : null;
  ...
  return (
    <ModalShell onClose={onClose} busy={busy} maxWidth={400}>
      ...
      >
        ⚠ Deducts XP and writes an audit entry to the ledger. The user will be
        notified. XP cannot go below 0.
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        >
          ...
          <AnimatePresence>
            {previewBalance !== null && (
              ...
              >
                <div style={{ fontSize: 22, color: "#cbd5e1", lineHeight: 1 }}>
                  →
                </div>
                <div style={{ textAlign: "center" }}>
                  <div
        ...
        <div>
          ...
          <textarea
            ...
            rows={2}
            placeholder="e.g. Duplicate complaint spam, system correction…"
            style={{
              width: "100%",
              boxSizing: "border-box",
              padding: "10px 14px",
