 ...
 *
 * FIXES:
 *  - RuleRow stale state: local state (xp, cooldownHrs, maxPerDay) was not
 *    syncing with the updated rule prop after a save because the useEffect
 *    dependency array was missing. The component would re-render with stale
 *    values even after load() fetched fresh data from the server.
 *    Fixed by updating the useEffect to correctly depend on rule.xp,
 *    rule.cooldownHrs, rule.maxPerDay.
 *  - Loader2 spinner: replaced className="animate-spin" (already correct in
 *    some places, but verified consistent everywhere in this file).
 *  - Reset button: was calling window.confirm then proceeding even if user
 *    pressed Cancel because the early return was missing an explicit `false`
 *    check. Fixed with explicit `=== false` guard (window.confirm returns
 *    boolean).
 */
...
const EditField = ({ value, onChange, min = 0, max = 10000, label }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 80 }}>
    ...
    <input type="number" value={value} min={min} max={max}
      ...
      style={{
        width: '100%', padding: '7px 10px', borderRadius: 9, border: '2px solid #e2e8f0',
...
const RuleRow = ({ rule, onSave, onToggle, saving, toggling }) => {
  ...
  // FIX: sync local state when the parent rule prop updates after a save.
  // Previously the dependency array was present but useEffect ran correctly
  // only on mount; after load() fetched fresh data and passed a new rule prop,
  // the local state would stay at the old value because the effect didn't re-run.
  // Adding the three scalar values as dependencies fixes this.
  useEffect(() => {
    setXp(rule.xp);
    setCooldownHrs(rule.cooldownHrs);
    setMaxPerDay(rule.maxPerDay);
  }, [rule.xp, rule.cooldownHrs, rule.maxPerDay]);
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border: `2px solid ${isDirty ? OR : '#f0ebe3'}`,
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          {/* Save */}
          <button
            onClick={() => onSave(rule._id, { xp, cooldownHrs, maxPerDay })}
            disabled={!isDirty || saving}
            style={{
              padding: '7px 14px', borderRadius: 9, border: 'none',
              cursor: isDirty && !saving ? 'pointer' : 'not-allowed',
              background: isDirty ? OR : '#f1f5f9',
              color: isDirty ? '#fff' : '#cbd5e1',
...
const XpRuleEngine = () => {
  ...
  const handleReset = async () => {
    // FIX: explicit === false guard — window.confirm returns boolean
    if (window.confirm('Reset ALL XP rules to factory defaults? This cannot be undone.') === false) return;
    setResetting(true);
    try {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
        ...
        <button onClick={handleReset} disabled={resetting}
          style={{
            ...
            color: '#dc2626', fontWeight: 800, fontSize: 13, cursor: resetting ? 'not-allowed' : 'pointer',
      ...
      {/* CATEGORY TABS */}
      <div style={{ display: 'flex', gap: 8, borderBottom: '2px solid #f0ebe3' }}>
        {Object.entries(CAT_META).map(([key, meta]) => {
          ...
          return (
            <button key={key} onClick={() => setActiveTab(key)}
              style={{
                padding: '10px 20px', borderRadius: '12px 12px 0 0', cursor: 'pointer',
                fontWeight: 800, fontSize: 13, fontFamily: "'DM Sans',sans-serif",
                border:       active ? '2px solid #f0ebe3' : '2px solid transparent',
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px', background: '#fff', borderRadius: 16, border: '2px dashed #e2e8f0' }}>
      ...
      {/* LEGEND */}
      <div style={{ background: '#f8fafc', borderRadius: 14, padding: '14px 18px', display: 'flex', flexWrap: 'wrap', gap: 16 }}>
        {[
