 ...
 *
 * Global XP ledger — admin audit view.
 * Filters: userId, action, date range, pagination.
 * Shows summary bar: total given / total taken / net.
 * Route: GET /api/admin/xp-ledger
 */
...
// ── Action metadata (mirrors AuthContext ACTION_LABELS) ───────────────────────
const ACTION_META = {
...
const SummaryPill = ({ icon: Icon, label, value, color, bg }) => (
  <div style={{ background: bg, borderRadius: 14, padding: '14px 20px',
    ...
    <div>
      <div style={{ fontFamily: "'Fraunces',serif", fontWeight: 900,
        fontSize: 22, color, lineHeight: 1 }}>{value}</div>
...
const XpLedgerAudit = () => {
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { setPage(1); }, [userIdF, actionF, fromF, toF]);
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start',
        ...
        <div>
          <h2 style={{ fontFamily: "'Fraunces',serif", fontWeight: 900,
            fontSize: 24, color: NV, margin: 0 }}>XP Ledger Audit</h2>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center',
        ...
        {(userIdF || actionF || fromF || toF) && (
          <button onClick={() => { setUserIdF(''); setActionF(''); setFromF(''); setToF(''); }}
            style={{ padding: '9px 14px', borderRadius: 10, border: '2px solid #fee2e2',
          ...
          <>
            ...
            <div style={{ display: 'grid',
              ...
              {['User', 'Action', 'Note', 'XP', 'Balance', 'Date'].map(h => (
                <div key={h} style={{ fontSize: 11, fontWeight: 800, color: '#64748b',
