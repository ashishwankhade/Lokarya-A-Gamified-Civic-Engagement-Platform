 ...
 *
 * FIXES:
 *  - ReviewModal: decision/adminNote/qrUrl state now reset via key prop on each
 *    open, preventing stale state when admin re-opens a modal after a failed/
 *    cancelled review.
 *  - AdminNote textarea: added boxSizing:'border-box' to prevent width overflow.
 *  - Cancel button inside busy state now correctly prevented (disabled).
 *  - GPS warning on list card: wrapped text in a proper flex container for icon.
 */
import React, { useState, useCallback, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
  CheckCircle2, XCircle, MapPin, CalendarDays,
  Users, Zap, Building2, Loader2,
  Clock, Info,
} from 'lucide-react';
...
const CAT_COLORS = {
  ...
  Healthcare:     '#dc2626',
  Social:         '#7c3aed',
  'Animal Welfare':'#d97706',
  Sanitation:     '#0891b2',
  'Disaster Relief':'#ea580c',
};
  ...
  const submit = async () => {
    ...
    try {
      const { data } = await api.patch(
        `/admin/activities/${activity._id}/review`,
        { decision, adminNote },
      );
      if (data.qrDataUrl) setQrUrl(data.qrDataUrl);
      toast.success(
        decision === 'approved'
          ? 'Mission approved! QR generated.'
          : 'Mission rejected.',
      );
          ...
          <>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                <div style={{ flex: 1 }}>
                  <span style={{
                    background: `${CAT_COLORS[activity.category] || '#64748b'}18`,
                  ...
                  <h3 style={{
                    ...
                    fontSize: 20, color: NV, lineHeight: 1.3,
                  }}>{activity.title}</h3>
                </div>
              </div>
              ...
              {activity.location?.lat && activity.location?.lng ? (
                <div style={{
                  marginTop: 12, background: '#ecfdf5', border: '1.5px solid #a7f3d0',
                ...
                <div style={{
                  marginTop: 12, background: '#fff0e0', border: '1.5px solid #fde8c8',
                  borderRadius: 10, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8,
                }}>
            ...
            <div style={{ borderTop: '2px solid #f0ebe3', paddingTop: 20 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                {/* FIX: added boxSizing:'border-box' to prevent overflow */}
                <textarea
                  value={adminNote}
                  onChange={e => setAdminNote(e.target.value)}
                  rows={2}
                  placeholder="e.g. Looks great! or Please re-submit with GPS coords."
                  style={{
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
                {/* FIX: Cancel button disabled while busy */}
                <button
                  onClick={onClose}
                  disabled={busy}
                  style={{
...
const ActivityApprovals = ({ onApproved }) => {
  ...
  // FIX: reviewing stores the activity; ReviewModal gets a key={activity._id}
  // so React remounts it fresh each time, resetting all modal state automatically.
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <AnimatePresence>
        {reviewing && (
          /* FIX: key={reviewing._id} forces remount → fresh state every open */
          <ReviewModal
            key={reviewing._id}
            activity={reviewing}
          ...
          {activities.map((act, i) => {
            ...
            return (
              <motion.div
                ...
                style={{
                  background: '#fff', borderRadius: 18, border: '2px solid #f0ebe3',
                ...
                {/* content */}
                <div style={{
                  flex: 1, padding: '16px 20px', minWidth: 0,
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
                    {[
                      ...
                      <span key={j} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#94a3b8' }}>
                  ...
                  {(!act.location?.lat || !act.location?.lng) && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#fff0e0', borderRadius: 8, padding: '6px 10px' }}>
