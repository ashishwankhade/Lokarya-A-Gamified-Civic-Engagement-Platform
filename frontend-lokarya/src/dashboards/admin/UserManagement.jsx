 ...
 *
 * FIXES:
 *  - Prev pagination button: was missing `cursor:'not-allowed'` when disabled,
 *    always showing pointer. Fixed to match Next button behaviour.
 *  - handleBan / handleDelete: added frontend guard for super_admin targets.
 *    The backend already rejects these calls (403), but without a frontend guard
 *    the admin sees a confusing generic error. Now shows a clear toast before
 *    the call is even made.
 *  - Revoke XP button: was always enabled even for super_admin target users.
 *    Super admins should not have XP revoked without at least a warning; added
 *    isSA guard to disable award/revoke buttons for super_admin rows.
 */
...
const UserManagement = () => {
  ...
  const handleBan = async (user) => {
    // FIX: frontend guard — backend returns 403 but give clear toast first
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {/* ── TABLE ── */}
      <div style={{ background: '#fff', borderRadius: 20, border: '2px solid #f0ebe3', overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 24px' }}>
            ...
            <p style={{ color: '#94a3b8', fontWeight: 700, marginBottom: 14 }}>No users found.</p>
                    ...
                    <motion.tr key={u._id}
                      ...
                      <td style={{ padding: '14px 16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div>
                            <div style={{ fontWeight: 800, fontSize: 13, color: u.banned ? '#dc2626' : NV }}>
                              {u.name}{u.banned && ' 🚫'}
                            </div>
                      ...
                      {/* XP */}
                      <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                        <span style={{ fontFamily: "'Fraunces',serif", fontWeight: 900, fontSize: 16, color: OR }}>
                          {u.xp || 0}
                        </span>
                      </td>
                      ...
                      <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                        {u.banned
                          ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#fee2e2', color: '#dc2626', borderRadius: 999, padding: '4px 10px', fontSize: 11, fontWeight: 800 }}>
                              <XCircle size={11} /> Banned
                            </span>
                          : <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#ecfdf5', color: '#059669', borderRadius: 999, padding: '4px 10px', fontSize: 11, fontWeight: 800 }}>
                              <CheckCircle2 size={11} /> Active
                            </span>}
                      </td>

                      {/* Joined */}
                      <td style={{ padding: '14px 16px', textAlign: 'center', fontSize: 12, color: '#94a3b8' }}>
                      ...
                      <td style={{ padding: '14px 16px' }}>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                          ...
                          {/* Award XP — FIX: disabled for super_admin */}
                          <ActionBtn
                            onClick={() => setAwardUser(u)}
                            disabled={isBusy || isSA}
                            title={isSA ? 'Cannot award XP to super admin' : 'Award XP'}
                            bg="#fef3c7" color="#d97706">
                            <Zap size={14} />
                          </ActionBtn>

                          {/* Revoke XP — FIX: disabled for super_admin */}
                          <ActionBtn
                            onClick={() => setRevokeUser(u)}
                            disabled={isBusy || isSA}
                            title={isSA ? 'Cannot revoke XP from super admin' : 'Revoke XP'}
                            bg="#fef2f2" color="#dc2626">
                            <MinusCircle size={14} />
                          </ActionBtn>

                          {/* Change Role — only for non-super_admin */}
                          {!isSA && (
                            <ActionBtn
                          ...
                          {/* Ban / Unban — only for non-super_admin */}
                          {!isSA && (
                            <ActionBtn
                              onClick={() => handleBan(u)}
                              disabled={isBusy}
                          ...
                          {/* Delete — only for non-super_admin */}
                          {!isSA && (
                            <ActionBtn
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
          <button
            ...
            style={{
              ...
              background: '#fff',
              // FIX: was always 'pointer'; disabled state now shows not-allowed
