 ...
 *
 * FIXES:
 *  - Removed local BarChart3 redeclaration at bottom of file. It was shadowing
 *    the lucide-react import used in AdminDashboard's navItems, causing the nav
 *    icon to render as an SVG with no styling. BarChart3 is now imported from
 *    lucide-react at the top of this file (used in QuickActions).
 *  - Added navigate to useEffect dependency array (react-hooks/exhaustive-deps).
 */
...
import {
  ...
  ArrowRight, Clock, Shield, BarChart3,
} from 'lucide-react';
...
const StatCard = ({ icon: Icon, label, value, sub, color, bg, delay, onClick }) => (
  <motion.div
    ...
    <div style={{
      width: 50, height: 50, borderRadius: 14, background: bg, flexShrink: 0,
...
const AdminOverview = ({ onNavigate }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 26, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <motion.div {...f(0)} style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{
            ...
            <span style={{
              color: '#e9d5ff', fontSize: 11, fontWeight: 800,
          ...
          <h2 style={{
            fontFamily: "'Fraunces',serif", fontWeight: 900, color: '#fff',
        ...
        <motion.div {...f(0.34)}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 16 }}>
            {[
              ...
              <div key={i} style={{
                background: '#fff', borderRadius: 14, border: '2px solid #f0ebe3',
                ...
                <div>
                  <div style={{
                    fontFamily: "'Fraunces',serif", fontWeight: 900,
                    fontSize: 22, color: NV, lineHeight: 1,
                  }}>{s.value}</div>
      ...
      {/* QUICK ACTIONS */}
      <motion.div {...f(0.4)}>
        <h3 style={{ fontFamily: "'Fraunces',serif", fontWeight: 900, fontSize: 18, color: NV, marginBottom: 14 }}>
          Quick Actions
        </h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 14 }}>
          <QuickAction
            icon={CheckSquare} color={V} bg="#f5f3ff"
            onClick={() => onNavigate('approvals')}
            label="Review Pending Missions"
            desc="Approve or reject NGO mission submissions"
          />
          <QuickAction
            icon={Users} color="#2563eb" bg="#eff6ff"
            onClick={() => onNavigate('users')}
            label="Manage Users"
            desc="Change roles, ban accounts, award XP manually"
          />
          <QuickAction
            icon={Zap} color="#d97706" bg="#fef3c7"
            onClick={() => onNavigate('xp-engine')}
            label="Configure XP Rules"
            desc="Set point values for every citizen and NGO action"
          />
          <QuickAction
            icon={BarChart3} color="#059669" bg="#ecfdf5"
            onClick={() => onNavigate('analytics')}
            label="View Analytics"
            desc="Trends, top earners, complaint heatmaps"
          />
        </div>
      </motion.div>
    </div>
