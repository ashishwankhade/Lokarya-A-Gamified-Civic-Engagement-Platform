 ...
 *
 * FIXES:
 *  - Removed dead `getTier` function (defined but never called — TIER_MIN /
 *    TIER_NAMES are still used inline in the topEarners map, where the tier
 *    calculation is done directly without the helper, so the helper was truly
 *    unused dead code).
 *  - `<style>` tag was embedded inside JSX return causing the same CSS string
 *    to be injected into the DOM on every render. Moved to a module-level
 *    constant injected once via a top-level useEffect (standard pattern).
 *  - Added missing `period` to `useEffect` dependency comment (was already
 *    correct in code, just noted for clarity).
 */
...
// FIX: moved breakpoint CSS from an inline <style> tag (which re-injected on
// every render) to a module-level string injected once via useEffect.
const RESPONSIVE_CSS = `
  @media(max-width:860px){
    .analytics-grid-2,.analytics-grid-3,.analytics-grid-21{
...
const ChartTitle = ({ children, sub }) => (
  <div style={{ marginBottom: 18 }}>
    <h3 style={{ fontFamily: "'Fraunces',serif", fontWeight: 700, fontSize: 17, color: NV, marginBottom: 3 }}>
      {children}
    </h3>
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      background: '#fff', border: '2px solid #f0ebe3', borderRadius: 12,
...
const PlatformAnalytics = () => {
  ...
  // FIX: inject responsive CSS once on mount, not on every render
  useEffect(() => {
    const id  = 'analytics-responsive-css';
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 0', color: '#94a3b8', fontFamily: "'DM Sans',sans-serif" }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div className="analytics-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <motion.div {...f(0.05)}>
          <Card>
      ...
      <div className="analytics-grid-21" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
        <motion.div {...f(0.15)}>
          <Card>
                ...
                {vibhag.map((v, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 70, fontSize: 11, fontWeight: 700, color: '#64748b', flexShrink: 0, textAlign: 'right' }}>
                        {v.name}
                      </div>
                      <div style={{ flex: 1, height: 8, background: '#f1f5f9', borderRadius: 999 }}>
                        <div style={{ height: '100%', borderRadius: 999, width: `${pct}%`, background: `linear-gradient(to right,${OR},#fbbf24)` }} />
                      </div>
                      <span style={{ fontSize: 11, fontWeight: 800, color: OR, flexShrink: 0, minWidth: 24 }}>{v.count}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <Card>
            ...
            {topEarners.length === 0 ? <Empty /> : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {topEarners.map((u, i) => {
                  // FIX: inline tier calculation — removed dead getTier() helper
                  ...
                  return (
                    <div key={u._id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{
                        width: 22, fontFamily: "'Fraunces',serif", fontWeight: 900,
                        fontSize: 13, textAlign: 'right', flexShrink: 0,
                        color: i === 0 ? '#d97706' : i === 1 ? '#94a3b8' : '#cd7f32',
                      }}>{i + 1}</div>
                      <div style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, fontWeight: 800, color: NV, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {u.name}
                        </div>
                        <div style={{ fontSize: 10, color: '#94a3b8' }}>{tierName}</div>
                      </div>
                      <span style={{ fontFamily: "'Fraunces',serif", fontWeight: 900, fontSize: 15, color: OR, flexShrink: 0 }}>
                        {u.xp || 0}
                      </span>
                    </div>
                  );
